...
class VariableNumFeaturesEncoderV2(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat((x, 
                       0.5+torch.zeros(*x.shape[:-1], self.num_features - x.shape[-1], device=x.device),
                       torch.zeros(*x.shape[:-1], 1, device=x.device) + x.shape[-1]/self.num_features),-1)
        return self.base_encoder(x)

class VariableNumFeaturesEncoderV3(nn.Module):
    def __init__(self, base_encoder, num_features):
        super().__init__()
        self.base_encoder = base_encoder
        self.num_features = num_features
...
def get_variable_num_features_encoder_v2(encoder_creator):
    return lambda num_features, emsize: VariableNumFeaturesEncoderV2(encoder_creator(num_features+1, emsize), num_features)

def get_variable_num_features_encoder_v3(encoder_creator):
    return lambda num_features, emsize: VariableNumFeaturesEncoderV3(encoder_creator(num_features+1, emsize), num_features)

### Simple Encoders

def get_linear_y_encoder(emsize):
    return SequentialEncoder(
        #ConstantNormalizationInputEncoderStep(mean=.5, std=math.sqrt(1/12)),
...
def get_linear_x_encoder(emsize, features_per_group=1):
    return SequentialEncoder(
        ...
        NanHandlingEncoderStep(),
        LinearInputEncoderStep(
            num_features=features_per_group*2,
            emsize=emsize,
            in_keys=("main", "nan_indicators"),
        ),
    )
