...
from alphapfn.model.layer import PerFeatureLayer
...
class PerFeatureTransformer(nn.Module):
    ...
    def _forward(  # noqa: PLR0912, C901
        ...
        *,
        ...
        # When using cached KV (y is None), we should use the cache, not create new cache
        # single_eval_pos_ may be > 0 due to style embedding, but we still want to use cached KV
        use_cached_kv = self.cache_trainset_representation and single_eval_pos is None
        encoder_out = self.transformer_layers(
            embedded_input,
            single_eval_pos=single_eval_pos_ if not use_cached_kv else 0,
